...
def getConstantsEi(E,k):
  ...
  return q;
def transformElipse(e, q, p, corners,dx,dy,nx,ny,fk,k,E,A,B):
  cosT = np.cos(1)
  sinT = np.sin(1)
  ...
  q1 = q[2]*cosT+q[1]*sinT
  q2 = -q[2]*cosT+q[1]*sinT
  q3 = q[5]*(cosT**2)+ q[3]*(sinT**2)
  q4 = 0
  q5 = q[5]*(sinT**2)+q[3]*(cosT**2)
  ...
  ny = [dx[0],
          dx[1],
          dx[2]]
  return (e, q, p, corners,dx,dy,nx,ny,fk,k,E,A,B)
def transformHyperbola(e, q, p, corners,dx,dy,nx,ny,A,B):
  cosT = np.cos(1)
  sinT = np.sin(1)
...
def elipse (e, q, p, corners,dx,dy,nx,ny,fk,k,E,A,B):
  ...
  z = 0; 
  for i in range(0, 3):
    #print 2*(b[i] + c [i]),"wwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww"
    if (4*a[i]**2 - 4*(c[i] - b[i])*(b[i] + c[i])) >= 0 :
      if abs(b[i]+c[i] ) > epsilion:
        ...
        print 0
        e, q, p, corners,dx,dy,nx,ny,fk,k,E,A,B = transformElipse(e, q, p, corners,dx,dy,nx,ny,fk,k,E,A,B)
        A,B,v = elipse (e, q, p, corners,dx,dy,nx,ny,fk,k,E,A,B)
        return A,B,v
        #t[z] = 0
        #t[z+1] = 0
        #tStatus[z] = True
        #tStatus[z+1] = True
        #print t[z],"........................................",a[i],c[i],b[i]
    z += 2
  #print t, "***********************************"
  u = 2*np.arctan(t)
  #print u, "***********************************"
  ...
  for i in range(0, 3):
    if (b[i]**2 - 4*a[i]*c[i]) > 0 :
      if abs(a[i]) > epsilion:
        ...
        print 0
        e, q, p, corners,dx,dy,nx,ny,fk,k,E,A,B = transformHyperbola(e, q, p, corners,dx,dy,nx,ny,A,B)
        A,B,v = hyperbola (e, q, p, corners,dx,dy,nx,ny,A,B)
        return A,B,v
      z += 2
...
def degenerat(e, q, p, corners,dx,dy,nx,ny,A,B):
  #in case e is on the side of the triangel
  shift = 1
  b = [ny[0],
       ny[1],
       ny[2]]
...
def surfaces(e, q, p, corners,f,k,E):
  ...
  if q[3]*q[5] > 0 and q[2] == 0 and q[4] == 0 and q[1] == 0:
	  A,B,vn = elipse(e, q, p, corners,dx,dy,nx,ny,f,k,E,A,B)
  elif q[4] != 0 and (q[2],q[3],q[5] == 0,0,0 ):
    vn = hyperbola(e, q, p, corners, dx, dy, nx, ny,A,B)
  elif q[2] != 0 and q[3] != 0 and (q[1], q[4], q[5] == 0,0,0):
    vn = parabola(e, q, p, corners, dx, dy, nx, ny,A,B)
  elif q[1] != 0 and (q[2],q[3], q[4], q[5] == 0,0,0,0):
    vn = straightLine(e, q, p, corners, dx, dy, nx, ny,A,B)
  elif q[3] != 0 and (q[1],q[2], q[4], q[5] == 0,0,0,0):
    vn = degenerat(e, q, p, corners, dx, dy, nx, ny,A,B)
  else :
  	return 0;
  sum = 0.0
  print A , "AAA"
